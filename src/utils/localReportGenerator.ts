...
### REPORT ID: ASIC-SPEC-${processNode.toUpperCase()}-${Math.floor(Math.random() * 90000 + 10000)}
...
This document reviews the RTL and floorplan specifications of the high-performance ${processNode} custom ASIC core accelerator optimized for running **${workloadLabel}** workloads.

* **Target Compute Core:** ${outputs.macCount} MAC units running at **${outputs.frequencyGhz} GHz**
* **Numeric Format / Precision:** ${precision}
* **Weight/Activation Sparsity:** ${sparsity.toUpperCase().replace(/_/g, " ")}
* **System DRAM Protocol:** ${memoryType}
* **On-Chip Interconnect Network:** ${interconnectType.toUpperCase().replace(/_/g, " ")}
...
\`\`\`verilog
// TOP-LEVEL PARAMETERIZED ASIC RTL INTERFACE (Verilog 2001)
module asic_accelerator_top #(
  ...
  // Parameter check
  initial begin
    $display("[ASIC Spec] Instantiating %0d x %0d Systolic Matrix Core", MAC_COLS, MAC_ROWS);
    $display("[ASIC Spec] Integrated Scratchpad Memory Size: %0d Megabytes", SRAM_SZ_MB);
  end
