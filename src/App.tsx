...
import { AsicSpecSheet } from "./components/AsicSpecSheet";
...
import { Cpu, FileText, Terminal, AlertTriangle, CpuIcon, BookOpen, Home, Mail, Tv, Flame } from "lucide-react";
import { KnowledgeHub } from "./components/KnowledgeHub";
import { ErrorBoundary } from "./components/ErrorBoundary";
import { HomePage } from "./components/HomePage";
import { ContactUs } from "./components/ContactUs";
import { SiliconEngineeringVideoPlayer } from "./components/SiliconEngineeringVideoPlayer";
import { ThermalMapVisualizer } from "./components/ThermalMapVisualizer";
...
export default function App() {
  ...
  const [activeMiddleTab, setActiveMiddleTab] = useState<"estimates" | "roofline" | "wafer" | "registers" | "thermal">("estimates");
  
  // 4. App-level navigation state
  const [currentView, setCurrentView] = useState<"home" | "workspace" | "knowledge" | "video" | "contact">("home");

  const handleNavigate = (
    view: "home" | "workspace" | "knowledge" | "video" | "contact",
    middleTab?: "estimates" | "roofline" | "wafer" | "registers" | "thermal",
  ...
  return (
    <div id="ai-silicon-workspace" className="min-h-screen bg-slate-950 text-slate-100 flex flex-col antialiased">
        ...
        <div className="flex items-center gap-3 md:flex-1 justify-start w-full md:w-auto">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 font-mono">
              ASIC Core Selection & Silicon Accelerator Synthesis
            </p>
          </div>
        </div>
              ...
              <div className="flex-1 overflow-x-auto pb-2 lg:pb-0 scrollbar-none flex items-center justify-start lg:justify-end gap-2 text-left font-mono">
                {[
                  ...
                  },
                  {
                    ...
                    desc: "DVFS & Dynamic Heat",
                    middleTab: "thermal",
                    rightTab: "report",
                    active: activeMiddleTab === "thermal",
                    color: "border-orange-500/50 text-orange-400 hover:bg-orange-500/5"
                  },
                  {
                    num: "5",
            ...
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
              ...
              <div className="xl:col-span-4 flex flex-col min-h-[500px] xl:min-h-0 bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-lg">
                ...
                <div className="flex border-b border-slate-800 bg-slate-950 text-slate-400">
                  ...
                  <button
                    onClick={() => setActiveMiddleTab("thermal")}
                    className={`flex-1 flex flex-col items-center justify-center py-2.5 text-[9px] font-mono font-bold border-b-2 transition-all uppercase ${activeMiddleTab === "thermal" ? "border-orange-500 text-orange-400 bg-slate-900/40" : "border-transparent hover:text-slate-200"}`}
                  >
                ...
                <div className="flex-1 p-5 overflow-y-auto">
                  {activeMiddleTab === "estimates" && (
                    <div className="space-y-4">
                      <h3 className="text-sm font-bold font-mono text-slate-300 flex items-center gap-1.5 border-b border-slate-800 pb-2 uppercase">
                        <CpuIcon className="w-4 h-4 text-emerald-400" /> Peak ASIC Core Estimations
                      </h3>
                      <AsicSpecSheet inputs={inputs} outputs={outputs} />
                    </div>
                  )}
                  ...
                  {activeMiddleTab === "thermal" && (
                    <div className="space-y-2">
                      <ThermalMapVisualizer inputs={inputs} outputs={outputs} />
                    </div>
                  )}
      ...
      <footer className="border-t border-slate-900 bg-slate-950 px-6 py-3 flex flex-col sm:flex-row justify-between items-center text-[10px] text-slate-600 font-mono gap-2 shrink-0">
        <span>COREPICK ARCHITECT • ASIC CORE SELECTION & ACCELERATOR SYNTHESIS</span>
