...
interface AsicSpecSheetProps {
  inputs: WorkloadInputs;
  outputs: EstimationOutputs;
}

export const AsicSpecSheet: React.FC<AsicSpecSheetProps> = ({ inputs, outputs }) => {
  const {
    requiredTops,
    frequencyGhz,
    utilization,
  ...
  return (
    <div id="asic-spec-sheet-root" className="space-y-6">
      ...
      {/* Microarchitecture Settings Overview */}
      <div className="bg-slate-950/60 border border-slate-800/80 rounded-xl p-3.5 px-4 font-mono text-[10px] flex flex-wrap gap-x-6 gap-y-2 justify-between items-center text-slate-400">
        <div className="flex items-center gap-1.5">
          <span className="text-cyan-400 font-bold">PRECISION:</span>
          <span className="text-slate-200 font-semibold bg-slate-900 px-1.5 py-0.5 rounded border border-slate-800">{inputs.precision || "INT8"}</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="text-cyan-400 font-bold">SPARSITY:</span>
          <span className="text-slate-200 font-semibold bg-slate-900 px-1.5 py-0.5 rounded border border-slate-800 uppercase">{inputs.sparsity === "structured_2_4" ? "Structured 2:4" : inputs.sparsity === "unstructured_50" ? "Unstructured 50%" : "Dense"}</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="text-cyan-400 font-bold">MEMORY PHY:</span>
          <span className="text-slate-200 font-semibold bg-slate-900 px-1.5 py-0.5 rounded border border-slate-800">{inputs.memoryType || "LPDDR5X"}</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="text-cyan-400 font-bold">INTERCONNECT:</span>
          <span className="text-slate-200 font-semibold bg-slate-900 px-1.5 py-0.5 rounded border border-slate-800 uppercase">{inputs.interconnectType === "mesh_noc" ? "Mesh NoC" : inputs.interconnectType === "shared_bus" ? "Shared Bus" : "Ring Bus"}</span>
        </div>
      </div>
        ...
        <div className="space-y-3 font-mono text-xs">
          <div>
            <div className="flex justify-between text-slate-400 mb-1">
              <span>ASIC Core Dynamic Power</span>
