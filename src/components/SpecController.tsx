...
import { Sparkles, Sliders, Cpu, MessageSquare, Zap, Layers, BookOpen, Database, Network, Percent } from "lucide-react";
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              >
                ...
                <option value="7nm">7nm FinFET (Consumer Silicon standard)</option>
            ...
            {/* ADVANCED ARCHITECTURAL TUNING SECTION */}
            <div className="pt-4 border-t border-slate-800 space-y-4">
              <span className="text-[9px] font-mono font-bold px-2 py-0.5 rounded bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 uppercase">
                Silicon Microarchitecture Controls
              </span>

              {/* A. Numeric Precision */}
              <div>
                <label className="block text-[10px] font-mono font-bold text-slate-400 mb-1.5 flex items-center gap-1">
                  <span className="text-cyan-400 font-bold">#</span> NUMERIC DATA PRECISION
                </label>
                <select
                  value={inputs.precision || "INT8"}
                  onChange={(e) => updateField("precision", e.target.value)}
                  className="w-full bg-slate-950 border border-slate-800 text-slate-300 font-mono text-xs p-2.5 rounded-lg outline-none focus:border-cyan-500/50"
                >
                  <option value="FP16">FP16 (High dynamic range float, 16-bit)</option>
                  <option value="BF16">BF16 (Brain Float, high range, 16-bit)</option>
                  <option value="INT8">INT8 (Quantized integer standard, 8-bit)</option>
                  <option value="INT4">INT4 (Ultra-compressed quantized, 4-bit)</option>
                </select>
              </div>
              ...
              <div>
                <label className="block text-[10px] font-mono font-bold text-slate-400 mb-1.5 flex items-center gap-1">
                  <Percent className="w-3.5 h-3.5 text-cyan-400" /> WEIGHT & ACTIVATION SPARSITY
                </label>
                <select
                  value={inputs.sparsity || "none"}
                  onChange={(e) => updateField("sparsity", e.target.value)}
                  className="w-full bg-slate-950 border border-slate-800 text-slate-300 font-mono text-xs p-2.5 rounded-lg outline-none focus:border-cyan-500/50"
                >
                  <option value="none">Dense (No Sparsity optimizations)</option>
                  <option value="structured_2_4">Structured 2:4 Sparsity (NVIDIA-style, 2x compute skip)</option>
                  <option value="unstructured_50">Unstructured Sparsity (50% random zero pruning)</option>
                </select>
              </div>
              ...
              <div>
                <label className="block text-[10px] font-mono font-bold text-slate-400 mb-1.5 flex items-center gap-1">
                  <Database className="w-3.5 h-3.5 text-cyan-400" /> SYSTEM DRAM MEMORY PROTOCOL
                </label>
                <select
                  value={inputs.memoryType || "LPDDR5X"}
                  onChange={(e) => updateField("memoryType", e.target.value)}
                  className="w-full bg-slate-950 border border-slate-800 text-slate-300 font-mono text-xs p-2.5 rounded-lg outline-none focus:border-cyan-500/50"
                >
                  <option value="LPDDR4X">LPDDR4X (Mobile, 4266 Mbps baseline)</option>
                  <option value="LPDDR5X">LPDDR5X (High-efficiency Mobile, 8533 Mbps)</option>
                  <option value="GDDR6">GDDR6 (High-throughput Graphics SRAM-bus, 16 Gbps)</option>
                  <option value="HBM3">HBM3 (Extreme Stacked Silicon-Interposer, 819 GB/s)</option>
                </select>
              </div>
              ...
              <div>
                <label className="block text-[10px] font-mono font-bold text-slate-400 mb-1.5 flex items-center gap-1">
                  <Network className="w-3.5 h-3.5 text-cyan-400" /> ON-CHIP CORE INTERCONNECT
                </label>
                <select
                  value={inputs.interconnectType || "mesh_noc"}
                  onChange={(e) => updateField("interconnectType", e.target.value)}
                  className="w-full bg-slate-950 border border-slate-800 text-slate-300 font-mono text-xs p-2.5 rounded-lg outline-none focus:border-cyan-500/50"
                >
                  <option value="shared_bus">Shared Central Bus (Simple, high congestion)</option>
                  <option value="hierarchical_ring">Hierarchical Ring Bus (Balanced scalability)</option>
                  <option value="mesh_noc">Network-on-Chip (NoC) Packet Mesh (Extreme scalability)</option>
                </select>
              </div>
            </div>
