...
export const KnowledgeHub: React.FC = () => {
  ...
  return (
    <div id="knowledge-hub-root" className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-fade-in">
      ...
      {/* HEADER CONTROLLER */}
      <div className="col-span-12 bg-slate-900 border border-slate-800 rounded-xl p-4 md:p-5 flex flex-col md:flex-row justify-between items-center gap-4 shadow-xl">
        <div className="space-y-1 text-left">
          <div className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-indigo-500 animate-pulse"></span>
            <span className="text-[10px] font-bold font-mono tracking-wider text-indigo-400 uppercase">Interactive Academy</span>
          </div>
          <h2 className="text-sm font-black font-mono text-white flex items-center gap-2">
            Silicon Engineering Learning Hub & Publications
          </h2>
          <p className="text-[10px] text-slate-400 font-mono leading-relaxed">
            Read formal academic microarchitectural specifications and use our diagnostic compliance auditor to evaluate pre-silicon design quality.
          </p>
        </div>
      </div>
          ...
          {activeArticleId === "systolic-arrays" && (
            <div className="space-y-3">
              ...
              <p className="text-xs text-slate-400 font-mono leading-relaxed">
                Modern silicon accelerators rely on 2D grids of Multiply-Accumulate (MAC) elements to speed up massive matrix multiplication (GEMM). To minimize memory bandwidth requirements, systolic arrays pipe intermediate outputs directly to adjacent elements, rather than back to centralized registers.
              </p>
...
endgroup`}
          ...
          {activeArticleId === "thermal-hotspots" && (
            <div className="space-y-4">
              ...
              <p className="text-xs text-slate-400 font-mono leading-relaxed">
                In modern deep-learning ASICs, millions of MACs pulsing at multi-gigahertz clocks create severe thermal gradients. Heat dissipation cannot keep up with localized dynamic power bursts. Left unchecked, local junction temperatures can exceed 125°C, triggering permanent gate oxide degradation or immediate thermal shutdown.
              </p>
